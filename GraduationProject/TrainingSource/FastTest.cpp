...
void getHogDescriptor(vector<float>& descriptorVector) {
	ifstream fileopen;
	fileopen.open("genfiles/descriptorvector.dat");
	
	while (!fileopen.eof()) {
		float in;
		fileopen >> in;
		descriptorVector.push_back(in);
	}
}
int main(int argc, char** argv) {
	...
	unsigned long overallSamples = positiveTrainingImages.size() + negativeTrainingImages.size();
		...
		getHogDescriptor(descriptorVector);
		const double hitThreshold = 1.581704974174500;
