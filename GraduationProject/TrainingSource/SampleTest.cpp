...
static void detectTrainingSetTest(const HOGDescriptor& hog, const double hitThreshold, const vector<string>& posFileNames, const vector<string>& negFileNames) {
	...
	outFile << truePositives << " " << trueNegatives << " " << falsePositives << " " << falseNegatives << " " << endl;
}
...
int* shupple(int total) {
	...
	for (int j = 0; j < 1000; j++) {
		int a = rand() % total; int b = rand() % total;
		int temp;
		temp = arr[a];
		arr[a] = arr[b];
		arr[b] = temp;
...
int main(int argc, char** argv) {
	...
	for (int z = 0; z < 100; z++) {
		int* posArr = shupple(positiveTrainingImages.size());
		int* negArr = shupple(negativeTrainingImages.size());

		vector<string> posTrain;
		vector<string> posTest;
		vector<string> negTrain;
		vector<string> negTest;
		for (int i = 0; i < positiveTrainingImages.size(); i++) {
			if (posArr[i]) {
				posTrain.push_back(positiveTrainingImages.at(i));
			}
			else {
				posTest.push_back(positiveTrainingImages.at(i));
			}
		}
		for (int i = 0; i < negativeTrainingImages.size(); i++) {
			if (negArr[i]) {
				negTrain.push_back(negativeTrainingImages.at(i));
			}
			else {
				negTest.push_back(negativeTrainingImages.at(i));
			}
		}
		/// Retrieve the descriptor vectors from the samples
		unsigned long overallSamples = posTrain.size() + negTrain.size();
		// </editor-fold>
		...
		if (File.good() && File.is_open()) {
			...
			for (unsigned long currentFile = 0; currentFile < overallSamples; ++currentFile) {
				...
				const string currentImageFile = (currentFile < posTrain.size() ? posTrain.at(currentFile) : negTrain.at(currentFile - posTrain.size()));
				// Output progress
				...
				if (!featureVector.empty()) {
					 ...
					 */
					File << ((currentFile < posTrain.size()) ? "+1" : "-1");
		...
		detectTrainingSetTest(hog, hitThreshold, posTest, negTest);
		/*
		...
		detectTest(hog,hitThreshold, frame);
